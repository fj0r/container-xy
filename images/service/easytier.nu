...
export def main [context: record = {}] {
    ...
    | build {|ctx|
        ...
        b with-mount {
            ...
            match $role {
                "server" => {
                    ...
                    # Note: pass listeners via CLI (-l), not the TOML file. The
                    # easytier-core version shipped in this image (2.x) silently
                    # ignores the toml `listeners` key, while -l works reliably.
        ...
        b with-mount {
            ...
            #
            # NOTE: this image's easytier-core (2.x) silently ignores the toml
            # `listeners` key. Pass relay listeners on the command line instead,
            # e.g. `-l tcp://0.0.0.0:11010 -l udp://0.0.0.0:11010`.
            instance_name = "server"
            ...
            # listeners = ["tcp://0.0.0.0:11010", "udp://0.0.0.0:11010"]  # newer versions only
